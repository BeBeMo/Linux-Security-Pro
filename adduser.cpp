    ...
    :QWidget(parent)
{
   ...
   QHBoxLayout *hLayout4 = new QHBoxLayout(widget);
    ...
    add_pushbutton = new QPushButton("添加");
         ...
         else
         {
              ...
              mypwdfile.close();
                ...
                QByteArray ba = add_passwd.toLatin1();
                   char *mm = ba.data();
                QFile fromFile("/tmp/passwd.txt");
                fromFile.open(QIODevice::WriteOnly);
                fromFile.write(mm);
                fromFile.close();
                 QProcess *thread = new QProcess;
                 thread->start("bash add_user.sh "+ pwd + " "+name);
                 ...
                 {
                     QMessageBox::warning(this,"提示","新用户添加成功！");
                     widget->close();
                 }
         }
    }
